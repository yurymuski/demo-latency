...
echo "Hello Demo ${DOMAIN}" > /opt/nginx/files/index.html
...
server {
        ...
        location /hello {
                return 200 "hello nginx tuned config ${DOMAIN} server\n";
                add_header alt-svc 'h3-29=":443"; ma=86400';
...
docker run --name nginx --restart always -d -p 80:80 -p 443:443/tcp -p 443:443/udp -v /etc/letsencrypt/:/opt/nginx/certs/ -v /opt/nginx/conf/demo.conf:/etc/nginx/conf.d/demo.conf -v /opt/nginx/conf/nginx.conf:/etc/nginx/nginx.conf -v /opt/nginx/files/:/usr/share/nginx/html/ ymuski/nginx-quic
